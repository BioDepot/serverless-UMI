...
def invokeFunctions (bucket,topicId,work_dir,alignsDir,recv_topic,suffix,uploadDir,project_id,max_workers,startTimeout,finishTiemout):
    ...
    while splitFiles and alignAttempts < maxAlignAttempts:
        unstartedFiles=startInvoke(project_id,work_dir,alignsDir,splitFiles,bucket,topicId,recv_topic,uploadDir,max_workers,clearQueue=(alignAttempts==0))
        if unstartedFiles:
            sys.stderr.write("unable to put {} onto the start message queue\n".format(unstartedFiles))
        sys.stderr.write('Time elapsed for launch is {}\n'.format(timer()-start))
        splitFiles=waitOnFunctions(splitFiles,recv_topic,project_id,bucket,topicId, work_dir,alignsDir,uploadDir,startTimeout,finishTimeout)
